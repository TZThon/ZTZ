...
UPSTREAM_REPO_BRANCH = "ZeZo"

REPO_REMOTE_NAME = "temponame"
IFFUCI_ACTIVE_BRANCH_NAME = "ZeZo"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/ZeZo"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/ZeZo", force=True)
    ...
    try:
        remote.push("ZeZo:main", force=True)
    except Exception as error:
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        repo.create_head("ZeZo", origin.refs.master)
        repo.heads.ZeZo.set_tracking_branch(origin.refs.master)
        repo.heads.ZeZo.checkout(True)
