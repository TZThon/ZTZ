...
class Katm(BASE):
    ...
    ktm_id = Column(UnicodeText, primary_key=True, nullable=False)
...
def get_katm(chat_id, ktm_id):
    try:
        return SESSION.query(Katm).get((str(chat_id), ktm_id))
    finally:
        SESSION.close()
...
def add_katm(chat_id, ktm_id, f_name, f_reason):
    ...
    if not to_check:
        adder = Katm(str(chat_id), ktm_id, f_name, f_reason)
        SESSION.add(adder)
        SESSION.commit()
        return True
    rem = SESSION.query(Katm).get((str(chat_id), ktm_id))
    SESSION.delete(rem)
    SESSION.commit()
    adder = Katm(str(chat_id), ktm_id, f_name, f_reason)
    SESSION.add(adder)
